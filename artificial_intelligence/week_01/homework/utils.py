...
def name(object):
    ...
    return (
        getattr(object, "name", 0)
        or getattr(object, "__name__", 0)
...
def print_table(table, header=None, sep="   ", numfmt="%g"):
    ...
    if header:
        table = [header] + table
    table = [
        [if_(isnumber(x), lambda: numfmt % x, lambda: x) for x in row] for row in table
    ]
...
def AIMAFile(components, mode="r"):
    ...
    return open(os.path.join(*components), mode)
